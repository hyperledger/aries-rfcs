...
The stories focus on the daily lifecycle activities of a single individual who needs to open a brokerage account and update a Life Insurance Policy.
...
sends Stacey an offer for a *Brokerage Membership Credential* which she accepts. The firm also sends her an `evidence_access_request` and an explanation that the firm's policy for regulatory reasons is to obtain access to the proof that KYC due-diligence was performed for Address, Identity and Photo. Stacey uses her Wallet App to instruct her Cloud Agent to send an `evidence_access_response`.
