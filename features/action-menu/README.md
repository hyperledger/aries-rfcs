# Aries RFC 0000: Action Menu Protocol 0.9
...
Discovery of a peer Agent's capabilities or service offerings is currently reliant on knowledge obtained out-of-band.  There is no in-band DIDComm supported protocol for querying a peer to obtain a human freindly menu of their capabilities or service offerings. Whilst this protocol doesn't offer ledger wide discovery capabilities, it will allow a User Agent connected to another Agent, to present a navigable menu and request offered services. The protocol also provides an interface definition language to define action menu display, selection and request submission.
...
The protocol can be initiated by either the `requester` asking for a Menu or the `responder` sending an unsolicited Menu.  The protocol can end via the `requester` issuing an explicit Close operation or an internal timeout on the `responder` causing it to discard Menu context.  At any time a `requester` can obtain protocol status by requesting the current active Menu from a `responder`.
...
A `requestor` is expected to display only one active _menu_ per connection when action menus are employed by the `responder`. This _menu_ may be dismissed or re-requested by the `requestor`. A newly received _menu_ is not expected to interrupt a user, but rather be made available for the user to inspect possible actions related to the `responder`.
...
* `title` -- plain text string to be shown at the top of the menu
* `description` -- plain text string to be shown in smaller text below the title bar
* `errormsg` -- optional plain text string sent to indicate that the last _perform_ request did not work as expected. The text is to be presented to the user in the title section
* `options` -- one or more available actions which the `responder` supports and may be requests in a _perform_ message.  
