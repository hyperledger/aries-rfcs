# 0690: Cross-Platform Credential Representation
...
## Summary
Aries Agent developers currently build their products without a standard method to rendering credentials for end users.
Without an standard method or observed convention is very common to have the best representation of an issued credential can only be found in an Agent that was developed side by side.

This RFC describes how the Aries community can use available technologies to define an standard method of rendering credentials that is both cross-agent and cross platform while also empowering Credentials issuers to see their existing brand along with the credentials they issue.
