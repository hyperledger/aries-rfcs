...
DIDComm protocols typically make a strong assumption about who is the "first mover" -- the role that must emit the protocol's initial message. For example, in [RFC 0037: Present Proof Protocol](../0037-present-proof/README.md), the `verifier` starts the interaction by sending a `request-presentation` message; the `prover` can't go first.
