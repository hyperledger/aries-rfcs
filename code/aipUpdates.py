...
parser.add_argument(
    '--version', '-v', help='The AIP version to display. Defaults to the current version(s) in the local file')
...
_aip_pat = re.compile(
    r'^[ \t]*#+[ \t]*Aries Interop Profile Version: ([1-9]*.[0-9]*.[0-9]*)?[ \t]*$')
    ...
    for line in lines:
        ...
        if aip_link:
            if aip_link.group(1) == version:
                ...
                lines = subprocess.run(['curl', '-L', '--silent', aip_link.group(2).replace(
                    'tree', 'raw')], stdout=subprocess.PIPE).stdout.decode('utf-8')
                return lines
...
for line in txt:
    ...
    if (rfc and ListVersionRFCs):
        ...
        changed = re.search(protocol, subprocess.run(
            ['git', 'diff', '--name-only', commit, 'main'], stdout=subprocess.PIPE).stdout.decode('utf-8'))
        ...
        if changed:
            ...
            if args.diffs:
                ...
                print(subprocess.run(
                    ['git', 'diff', commit, 'main', protocol], stdout=subprocess.PIPE).stdout.decode('utf-8'))
exit()
